...
| [`apps/backend/src/services/generationPipeline.ts`](./apps/backend/src/services/generationPipeline.ts) | Framework-agnostic ядро генерации `.docx`, вынесенное из `routes/generate.ts` для переиспользования в UI-роуте, bizproc-хендлере и webhook-раннере. Экспортирует `runGeneration({templateId, dealId, client, logger}): Promise<GenerationResult>` — те же 9 шагов (загрузка template+formulas+AppSettings → deal context → evaluate formulas → `buildDocxFromHtml` → `disk.storage.getforapp` → `uploadDiskFile` → опциональный `crm.item.update` по `UF_CRM_*` → опциональный `crm.timeline.comment.add` → возврат `{fileId, downloadUrl, fileName, formulas, binding, timeline, warnings[]}`). Детерминированные ошибки пробрасываются как `GenerationError` с `kind: 'template_not_found'\|'bad_deal_id'\|'deal_not_found'\|'deal_gateway'\|'docx_build_failed'\|'disk_gateway'\|'upload_failed'\|'unexpected'` — вызывающая сторона сама маппит их в HTTP-статусы или per-template записи. Экспортирует типы `GenerationResult`, `BindingResult`, `GenerationErrorKind`, `RunGenerationParams` и класс `GenerationError`. |
