...
| [`apps/backend/src/services/generationPipeline.ts`](./apps/backend/src/services/generationPipeline.ts) | Framework-agnostic ядро генерации `.docx`, вынесенное из `routes/generate.ts` для переиспользования в UI-роуте, bizproc-хендлере и webhook-раннере. Экспортирует `runGeneration({templateId, dealId, client, logger}): Promise<GenerationResult>` — шаги: загрузка template+formulas+AppSettings → `detectProductUsage` (сканирование HTML и формул на product-хелперы) → deal context с `fetchProducts`/`fetchProductImages` → evaluate formulas → `buildDocxFromHtml` (с `products: context.PRODUCTS`) → `disk.storage.getforapp` → `uploadDiskFile` → опциональный `crm.item.update` по `UF_CRM_*` → опциональный `crm.timeline.comment.add` → возврат `{fileId, downloadUrl, fileName, formulas, binding, timeline, warnings[]}`. Приватный хелпер `detectProductUsage(contentHtml, expressions)` определяет нужна ли загрузка товаров/картинок. Детерминированные ошибки пробрасываются как `GenerationError` с `kind`. Экспортирует типы `GenerationResult`, `BindingResult`, `GenerationErrorKind`, `RunGenerationParams` и класс `GenerationError`. |
