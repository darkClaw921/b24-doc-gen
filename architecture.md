...
| [`packages/shared/src/types.ts`](./packages/shared/src/types.ts) | Все интерфейсы: `AppSettings`, `Theme`, `Template`, `Formula`, `FormulaDependencies` (с опциональным `products?: boolean`), `DealField`, `EntityValues`, `ProductRow` (ID, PRODUCT_ID, PRODUCT_NAME, PRICE, QUANTITY, DISCOUNT_SUM, TAX_RATE, SUM, MEASURE_NAME, SORT, IMAGE_BASE64?, IMAGE_URL?), `FormulaContext` (DEAL, CONTACT, COMPANY, PRODUCTS: ProductRow[]), `FormulaEvaluationResult`, `B24AuthPayload`, `InstallRequest/Response`, `TemplatePreviewResponse`, `GenerateRequest/Response`, `WebhookSummary` (JSON-сериализованный снимок `Webhook` с полем `url = ${PUBLIC_URL}/api/webhook/run/${token}`, литеральный union `scope: 'theme' \| 'template'`, даты как ISO-строки), `ApiError` (legacy flat shape), `ApiErrorBody`/`ApiErrorEnvelope` (Фаза 6 — wrapped envelope от центрального error handler), `AppRole`. |
...
| [`apps/frontend/src/components/Editor/TiptapEditor.tsx`](./apps/frontend/src/components/Editor/TiptapEditor.tsx) | TipTap-обёртка. `useEditor` с расширениями StarterKit + Image (`allowBase64`) + Table (resizable) + TableRow/TableHeader/TableCell + `FormulaTag` (inline atom-узел для формул) + `ProductFieldSpan`/`ProductImageSpan`/`ProductIndexSpan` (atom-ноды для product-таблиц). Контролируемый props: `content`, `onChange`, `editable`, `placeholder`. Экспортирует `buildTiptapExtensions()` для повторного использования. Колбэк `onReady(editor)` отдаёт инстанс редактора наружу для тулбара. |
| [`apps/frontend/src/components/Editor/Toolbar.tsx`](./apps/frontend/src/components/Editor/Toolbar.tsx) | Тулбар форматирования. Принимает `editor: Editor \| null` и опциональный `onInsertFormula`. Кнопки: bold/italic/strike, Heading 1/2/Paragraph, bullet/ordered list, вставка таблицы 3x3, вставка картинки (prompt URL), кнопка Σ «Вставить формулу» (только если задан `onInsertFormula`), кнопка «Таблица товаров» (Package icon) с dropdown-выбором колонок (`ProductTableDropdown` — чекбоксы для index/name/price/quantity/sum/discount/tax/measure/image, генерирует `<table data-product-table>` с `<span data-product-field>/<span data-product-image>/<span data-product-index>` плейсхолдерами), undo/redo. Использует иконки lucide-react. |
| [`apps/frontend/src/components/Editor/ProductTableNode.ts`](./apps/frontend/src/components/Editor/ProductTableNode.ts) | Кастомные TipTap inline atom ноды для product-таблиц: `ProductFieldSpan` (парсит `<span data-product-field>`, рендерит emerald-pill с именем поля), `ProductImageSpan` (парсит `<span data-product-image>`, purple-pill), `ProductIndexSpan` (парсит `<span data-product-index>`, gray-pill с «#»). |
| [`apps/frontend/src/components/Editor/index.ts`](./apps/frontend/src/components/Editor/index.ts) | Реэкспорт `TiptapEditor`, `Toolbar`, `buildTiptapExtensions`, `ProductFieldSpan`, `ProductImageSpan`, `ProductIndexSpan` и связанных типов. |
...
| [`apps/backend/src/routes/generate.ts`](./apps/backend/src/routes/generate.ts) | `registerGenerateRoutes(app)` (Фаза 5) — `POST /api/generate { templateId, dealId }`. Конвейер: 1) загрузка template+formulas из Prisma и `AppSettings`, 2) `detectProductUsage(contentHtml, expressions)` — сканирование шаблона на `data-product-table`/`data-product-image` атрибуты и product-хелперы (`productSum/productCount/productGet/productImage`) в формулах, определение `fetchProducts`/`fetchProductImages` флагов, 3) `getDealContext(client, dealId, {fetchProducts, fetchProductImages})` — batch для DEAL+CONTACT+COMPANY+PRODUCTS, 4) `evaluateExpression` для каждой формулы → `formulaResults[tagKey]`, 5) ветвление: если `template.originalDocx` → `buildDocxFromTemplate` (docxtemplater, прямая подстановка), иначе fallback на `buildDocxFromHtml(contentHtml, {formulas, title})` → Buffer, 6) `disk.storage.getforapp` → `ROOT_OBJECT_ID`, 7) `b24Client.uploadDiskFile(folderId, fileName, buffer)`, 8) опциональный `crm.item.update` через `AppSettings.dealFieldBinding` (warning если не настроено), 9) `crm.timeline.comment.add` со ссылкой. Приватная функция `detectProductUsage(html, expressions)` возвращает `{fetchProducts, fetchProductImages}`. Возвращает `GenerateRouteResponse {fileId, fileName, downloadUrl, formulas, binding, timeline, warnings[]}`. Все сторонние шаги non-fatal — fatal только template/dealContext/upload. |
...
| [`apps/backend/src/services/generationPipeline.ts`](./apps/backend/src/services/generationPipeline.ts) | Framework-agnostic ядро генерации `.docx`, вынесенное из `routes/generate.ts` для переиспользования в UI-роуте, bizproc-хендлере и webhook-раннере. Экспортирует `runGeneration({templateId, dealId, client, logger}): Promise<GenerationResult>` — шаги: загрузка template+formulas+AppSettings → `detectProductUsage` (сканирование HTML и формул на product-хелперы) → deal context с `fetchProducts`/`fetchProductImages` → evaluate formulas → ветвление: если `template.originalDocx` → `buildDocxFromTemplate` (docxtemplater), иначе `buildDocxFromHtml` (с `products: context.PRODUCTS`) → `disk.storage.getforapp` → `uploadDiskFile` → опциональный `crm.item.update` по `UF_CRM_*` → опциональный `crm.timeline.comment.add` → возврат `{fileId, downloadUrl, fileName, formulas, binding, timeline, warnings[]}`. Приватный хелпер `detectProductUsage(contentHtml, expressions)` определяет нужна ли загрузка товаров/картинок. Детерминированные ошибки пробрасываются как `GenerationError` с `kind`. Экспортирует типы `GenerationResult`, `BindingResult`, `GenerationErrorKind`, `RunGenerationParams` и класс `GenerationError`. |
...
| Файл | Назначение |
|---|---|
| [`apps/backend/prisma/schema.prisma`](./apps/backend/prisma/schema.prisma) | Схема БД. Модели: `AppSettings`, `Theme`, `Template`, `Formula`, `Webhook`. Datasource `sqlite` → `file:./dev.db`. |
...
- **Template** — `id` (cuid), `name`, `themeId` → Theme, `contentHtml` (TipTap HTML), `originalDocx?` (Bytes), `formulas[]`, `webhooks[]`. Индекс по `themeId`.
